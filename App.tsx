...
// Translation map for administrative regions and provinces
const ADMIN_TRANSLATIONS: Record<string, string> = {
  'Kappadokien': 'Cappadocia',
  'Kozaklı district': 'Kozaklı',
};

// Comprehensive translation map for heritage types
...
const translateType = (type: string) => TYPE_TRANSLATIONS[type] || type;
const translateAdmin = (admin: string) => ADMIN_TRANSLATIONS[admin] || admin;
...
const getIconConfig = (types: string[] = [], isUnesco: boolean = false) => {
  ...
  } else if (allTypesStr.includes('museum') || allTypesStr.includes('madrasa') || allTypesStr.includes('complex')) {
...
const getProcessedSites = (): HeritageSite[] => {
  const aggregateMap = RAW_GEOJSON_DATA.features.reduce((acc, feature) => {
    ...
    // Use coordinates in key to treat same entities at different spots as unique points
    const key = `${feature.properties.item}_${lat.toFixed(6)}_${lng.toFixed(6)}`;
    ...
    if (!acc.has(key)) {
      acc.set(key, {
        ...
        admin: translateAdmin(feature.properties.adminLabel),
        isUnesco: false,
        externalLinks: {
...
const App: React.FC = () => {
  ...
  const dynamicCategories = useMemo(() => {
    ...
    return ["ALL", "NEARBY", ...Array.from(types).sort()];
  }, [processedSites]);

  const filteredSites = useMemo(() => {
    return processedSites.filter(s => {
      const matchesSearch = s.name.toLowerCase().includes(searchTerm.toLowerCase()) || s.admin.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  useEffect(() => {
    ...
    const clusterGroup = L.markerClusterGroup({
      ...
      iconCreateFunction: (cluster: any) => {
        ...
        let bgColor = unescoCount / total >= 0.5 ? '#d97706' : '#64748b';
        return L.divIcon({ html: `<div class="custom-cluster-div" style="background-color: ${bgColor};"><span>${total}</span></div>`, className: 'marker-cluster', iconSize: L.point(40, 40) });
      }
    });
  ...
  useEffect(() => {
    ...
    filteredSites.forEach(site => {
      ...
      const marker = L.marker(site.coords, { icon: markerIcon, isUnesco: site.isUnesco }).on('click', () => { setSelectedSiteId(site.id); setViewMode('map'); });
      if (showLabels) marker.bindTooltip(site.name, { permanent: true, direction: 'top', offset: [0, -20], className: 'custom-map-label' });
  ...
  const handleLocateUser = () => {
    ...
    navigator.geolocation.getCurrentPosition((pos) => {
      ...
      setViewMode('map');
      if (mapRef.current) {
        mapRef.current.flyTo(p, 13);
        if (userMarkerRef.current) mapRef.current.removeLayer(userMarkerRef.current);
        userMarkerRef.current = L.marker(p, { icon: L.divIcon({ className: 'user-location-marker', iconSize: [16, 16], iconAnchor: [8, 8] }) }).addTo(mapRef.current);
      }
    }, () => { setIsLocating(false); alert("Unable to get location"); }, { enableHighAccuracy: true });
  };
    ...
    <div className="flex flex-col md:flex-row h-full w-full bg-slate-50 font-sans text-slate-900 overflow-hidden select-none">
      ...
      <div className={`${viewMode === 'list' ? 'flex' : 'hidden md:flex'} flex-1 md:flex-none w-full md:w-[420px] bg-white shadow-2xl z-20 flex-col h-full border-r border-slate-100 overflow-hidden`}>
        <div className="p-6 pb-2 shrink-0">
          <div className="flex items-center gap-3 mb-6">
            <div className="w-10 h-10 bg-amber-500 rounded-2xl flex items-center justify-center text-white shadow-lg"><svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5a2.5 2.5 0 010-5 2.5 2.5 0 010 5z"/></svg></div>
            <div><h1 className="text-xl font-black tracking-tighter uppercase leading-none">DKM NEVŞEHİR</h1><p className="text-[10px] font-bold text-slate-400 tracking-widest mt-1">HERITAGE EXPLORER</p></div>
          </div>
          <div className="relative mb-4">
            <input type="text" placeholder="Search locations..." className="w-full pl-12 pr-5 py-4 bg-slate-100 border-none rounded-2xl text-base focus:ring-2 focus:ring-amber-500 focus:bg-white outline-none transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          ...
          <div className="mb-4">
            <div className="flex gap-2 overflow-x-auto pb-3 custom-scrollbar-x px-1">
              {dynamicCategories.map(cat => <button key={cat} onClick={() => setActiveCategory(cat)} className={`flex-shrink-0 px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all ${activeCategory === cat ? 'bg-amber-600 text-white shadow-lg' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>{cat}</button>)}
            </div>
          </div>
          <div className="flex justify-between items-center text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">
             <div className="flex items-center gap-2"><span className="w-2 h-2 bg-amber-500 rounded-full animate-pulse"></span>{filteredSites.length} SHOWN</div>
             <div className="opacity-60 text-amber-600 font-bold">TOTAL: {processedSites.length} UNIQUE SITES</div>
          </div>
        </div>
        <div className="flex-1 overflow-y-auto px-6 pb-2 space-y-1 scroll-smooth">
          {filteredSites.map(site => <SiteCard key={site.id} site={site} onClick={() => { setSelectedSiteId(site.id); setViewMode('map'); }} isActive={selectedSiteId === site.id} />)}
          {filteredSites.length === 0 && <div className="py-20 text-center text-slate-300 px-6 font-bold uppercase text-sm tracking-widest">No matching heritage found.</div>}
        </div>
      ...
      <div className={`${viewMode === 'map' ? 'block' : 'hidden md:block'} flex-1 relative h-full bg-slate-200 overflow-hidden`}>
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-3 z-[1000] pointer-events-auto">
          <button onClick={() => setViewMode('list')} className="md:hidden w-12 h-12 bg-white flex items-center justify-center rounded-2xl shadow-xl border"><svg className="w-6 h-6 text-slate-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16" /></svg></button>
          <button onClick={handleLocateUser} disabled={isLocating} className={`flex items-center gap-3 px-5 py-3 rounded-2xl shadow-2xl border transition-all ${isLocating ? 'bg-amber-100 text-amber-500' : 'bg-white text-slate-700 hover:bg-slate-50'}`}>
            <svg className={`w-5 h-5 ${isLocating ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 8c-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4-1.79-4-4-4zm8.94 3c-.49-4.38-3.96-7.85-8.34-8.34V1a1 1 0 00-2 0v1.66c-4.38.49-7.85 3.96-8.34 8.34H1a1 1 0 000 2h1.66c.49 4.38 3.96 7.85 8.34 8.34V23a1 1 0 002 0v-1.66c4.38-.49 7.85-3.96 8.34-8.34H23a1 1 0 000-2h-1.66z" /></svg>
            <span className="font-black text-[11px] uppercase tracking-wider">{isLocating ? 'Locating...' : 'Locate Me'}</span>
          </button>
          <div className="relative">
            <button onClick={() => setIsLayersMenuOpen(!isLayersMenuOpen)} className={`flex items-center gap-3 px-5 py-3 rounded-2xl shadow-2xl border transition-all ${isLayersMenuOpen ? 'bg-amber-600 text-white' : 'bg-white text-slate-700'}`}><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" /></svg><span className="font-black text-[11px] uppercase tracking-wider">Layers</span></button>
            {isLayersMenuOpen && <div className="absolute left-0 mt-3 bg-white border rounded-[24px] shadow-2xl p-2 w-48 animate-detail overflow-hidden flex flex-col gap-1">{(['standard', 'satellite', 'terrain'] as MapType[]).map(t => <button key={t} onClick={() => { setMapType(t); setIsLayersMenuOpen(false); }} className={`px-4 py-3 rounded-xl text-left text-[10px] font-black uppercase tracking-wider transition-all ${mapType === t ? 'bg-amber-50 text-amber-600' : 'hover:bg-slate-50 text-slate-600'}`}>{t} View</button>)}</div>}
          </div>
          <button onClick={() => setShowLabels(!showLabels)} className={`flex items-center gap-3 px-5 py-3 rounded-2xl shadow-2xl border transition-all ${showLabels ? 'bg-slate-900 text-white' : 'bg-white text-slate-700'}`}><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" /></svg><span className="font-black text-[11px] uppercase tracking-wider">{showLabels ? 'Hide Text' : 'Show Text'}</span></button>
        </div>
        {selectedSite && (
          <div className="fixed md:absolute bottom-0 right-0 left-0 md:left-auto md:bottom-8 md:right-8 z-[1100] p-6 pointer-events-none animate-detail">
            <div className="bg-white rounded-[40px] shadow-2xl overflow-hidden pointer-events-auto border w-full md:w-[420px]">
              <div className="relative">
                {selectedSite.image ? <img src={selectedSite.image.includes('Special:FilePath') ? selectedSite.image + '?width=600' : selectedSite.image} className="w-full h-56 object-cover" alt={selectedSite.name} /> : <div className="w-full h-40 bg-slate-50 flex items-center justify-center text-slate-200"><svg className="w-12 h-12" fill="currentColor" viewBox="0 0 24 24"><path d="M21 19V5c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2zM8.5 13.5l2.5 3.01L14.5 12l4.5 6H5l3.5-4.5z"/></svg></div>}
                <button onClick={() => setSelectedSiteId(null)} className="absolute top-4 right-4 w-10 h-10 bg-black/20 hover:bg-black/40 backdrop-blur rounded-full flex items-center justify-center text-white">✕</button>
              </div>
              <div className="p-8"><h2 className="text-2xl font-black mb-1 text-slate-900 leading-tight">{selectedSite.name}</h2><p className="text-xs font-bold text-amber-600 uppercase tracking-widest mb-4">{selectedSite.admin}, NEVŞEHİR</p><div className="flex flex-wrap gap-2 mb-8">{selectedSite.types.map(t => <span key={t} className="text-[9px] font-black bg-slate-50 border px-3 py-1 rounded-full text-slate-400 uppercase tracking-widest">{t}</span>)}</div><a href={selectedSite.externalLinks.wiki} target="_blank" rel="noreferrer" className="block text-center py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-black shadow-lg">Wikipedia</a></div>
            </div>
          </div>
        )}
      </div>
